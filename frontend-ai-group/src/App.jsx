    ...
    <div ref={containerRef} className="absolute inset-0 overflow-hidden bg-slate-50">
      ...
      >
        ...
        <g transform={`translate(${corePos.x}, ${corePos.y})`}>
          ...
          <text x="0" y="-30" textAnchor="middle" fill="#0891b2" fontSize="10" fontWeight="800" fontFamily="monospace" letterSpacing="1.5">
            CASE DATA ROOT
          </text>
        ...
        {agentPositions.map((agent) => {
          ...
          return (
            <g
              key={agent.id}
              style={{ transform: `translate(${agent.x}px, ${agent.y}px)`, cursor: "pointer", transition: "transform 0.6s cubic-bezier(0.4, 0, 0.2, 1)" }}
              onMouseEnter={() => setHoveredNode(agent.id)}
              onMouseLeave={() => setHoveredNode(null)}
              onClick={() => onNodeClick && onNodeClick(agent.name)}
            >
              <rect x={-nodeWidth / 2} y={-nodeHeight / 2} width={nodeWidth} height={nodeHeight} rx="12" fill="#ffffff" stroke={agent.color} strokeWidth={isActive ? 3 : 1.5} filter={`url(#glow-${agent.id})`} />
              ...
              <text x={-nodeWidth / 2 + 42} y="-2" fill="#0f172a" fontSize="10" fontWeight="800">
                {agent.name}
              </text>
              <text x={-nodeWidth / 2 + 42} y="14" fill="#64748b" fontSize="9" fontFamily="monospace">
                {agent.subtitle}
              </text>
            </g>
          );
...
function SimulationView({ simulationData, simulationResults, onRunSimulation, onConfirmSimulation, onSkipSimulation, onBackToDiscussion, wsConnected }) {
  ...
  const handleRun = () => {
    ...
    if (Object.keys(changed).length === 0) {
      ...
      setToast({ type: "info", message: "Running baseline simulation..." });
    } else {
        ...
        <div className="w-[420px] flex flex-col border-r border-slate-200 bg-white overflow-y-auto">
          <div className="p-6 border-b border-slate-100">
            ...
            <div className="relative">
              ...
              {(simulationData?.original_problem?.length > 150) && (
                <button onClick={() => setIsProblemExpanded(!isProblemExpanded)} className="text-[#4648d4] text-[10px] font-bold mt-1 hover:underline">
          ...
          <div className="p-6 flex-1">
            <div className="flex items-center justify-between mb-5">
              <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2"><SlidersHorizontal className="w-4 h-4 text-[#4648d4]" /> Parameters</h4>
              <button onClick={handleReset} className="text-xs text-slate-500 hover:text-[#4648d4] font-medium flex items-center gap-1"><RefreshCcw className="w-3 h-3" /> Reset</button>
            </div>
            {hasVariables ? (
            ...
            <div className="mt-6 space-y-3">
              <button onClick={handleRun} disabled={isRunning || !wsConnected} className="w-full py-3 bg-[#4648d4] hover:bg-[#3638b0] disabled:bg-slate-300 text-white rounded-xl font-semibold text-sm shadow-lg shadow-indigo-500/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2">
                {isRunning ? <RefreshCcw className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
                {isRunning ? "Running Simulation..." : "Run Simulation"}
              </button>
              <button onClick={onSkipSimulation} disabled={!wsConnected} className="w-full py-3 bg-slate-100 hover:bg-slate-200 disabled:bg-slate-50 text-slate-700 rounded-xl font-semibold text-sm border border-slate-200 transition-all active:scale-[0.98] flex items-center justify-center gap-2">
                <FastForward className="w-4 h-4" /> Skip to Action Plan
              </button>
            </div>
          </div>
          <div className="p-6 border-t border-slate-100 bg-slate-50/50">
            <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2 mb-3"><Brain className="w-4 h-4 text-[#4648d4]" /> Expert Analysis</h4>
                  ...
                  if (roleMatch) {
                    return (
                      <div key={i} className="mb-2">
                        <div className="font-semibold text-[#4648d4] mt-2">[{roleMatch[1]}]:</div>
                        <div className="pl-2 border-l-2 border-slate-200 text-xs text-slate-600 leading-relaxed">{roleMatch[2]}</div>
                      </div>
                    );
                  }
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
          {!simulationResults ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-slate-500 max-w-md">Adjust parameters and run the simulation, or skip directly to the Action Plan.</p>
            </div>
          ) : (
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className={`rounded-2xl p-4 border flex items-start gap-3 ${getRiskStyles(simulationResults.operational_impact.risk_level).bg}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className={`text-xs space-y-1 ${getRiskStyles(simulationResults.operational_impact.risk_level).text}`}>
                      {simulationResults.operational_impact.warnings?.map((w, i) => <div key={i} className="flex items-start gap-1.5"><span className="mt-1 w-1 h-1 rounded-full bg-current shrink-0" /><span>{w}</span></div>)}
                    </div>
                    {simulationResults.operational_impact.recommendations?.length > 0 && (
                      <div className={`text-xs mt-2 pt-2 border-t border-black/5 space-y-1 ${getRiskStyles(simulationResults.operational_impact.risk_level).text} opacity-80`}>
                        {simulationResults.operational_impact.recommendations.map((r, i) => <div key={i} className="flex items-start gap-1.5"><Lightbulb className="w-3 h-3 mt-0.5 shrink-0" /><span>{r}</span></div>)}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2"><BarChart3 className="w-4 h-4 text-[#4648d4]" /> Key Performance Indicators</h3>
                  ...
                  {simulationResults.kpis && Object.keys(simulationResults.kpis).length > 0 ? (
                    Object.entries(simulationResults.kpis).map(([key, data]) => (
                      <div key={key} className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm">
                        <div className="flex items-center gap-2 mb-3"><div className="w-8 h-8 rounded-lg bg-[#4648d4]/10 flex items-center justify-center"><TrendingUp className="w-4 h-4 text-[#4648d4]" /></div><span className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">{formatKpiName(key)}</span></div>
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-100"><p className="text-[10px] text-slate-400 font-mono truncate" title={data.formula}>{data.formula}</p></div>
                      </div>
                    ))
                  ) : (
                    <div className="col-span-3 text-center py-8 text-slate-400 text-sm bg-white rounded-2xl border border-slate-200"><BarChart3 className="w-6 h-6 mx-auto mb-2 opacity-40" /><p>No KPIs available for this domain</p></div>
                  )}
                </div>
              </div>
              <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2"><Calculator className="w-4 h-4 text-[#4648d4]" /> Base Calculations</h3>
                  ...
                  {simulationResults.base_calculations && Object.keys(simulationResults.base_calculations).length > 0 ? (
                    Object.entries(simulationResults.base_calculations).map(([key, data]) => (
                      <div key={key} className="flex items-center gap-4 p-3 rounded-xl bg-slate-50 border border-slate-100">
                        <div className="w-10 h-10 rounded-lg bg-[#4648d4]/10 flex items-center justify-center shrink-0"><span className="text-[#4648d4] font-bold text-sm">=</span></div>
                        <div className="flex-1 min-w-0"><p className="text-xs font-semibold text-slate-900 uppercase tracking-wide">{formatKpiName(key)}</p><p className="text-xs text-slate-500 font-mono mt-0.5">{data.formula} = {data.substitution}</p></div>
                        <div className="text-right shrink-0"><p className="text-lg font-bold text-[#4648d4] font-mono">{formatNumber(data.result)}</p><p className="text-[10px] text-slate-500">{data.unit}</p></div>
                      </div>
                    ))
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2"><Scale className="w-4 h-4 text-[#4648d4]" /> Scenario Comparison</h3>
                <div className="space-y-5">
                    ...
                    Object.entries(simulationResults.scenario_comparison).map(([key, data]) => {
                      ...
                      return (
                        <div key={key} className="space-y-2">
                          <div className="flex items-center justify-between"><span className="text-sm font-semibold text-slate-900">{formatKpiName(key)}</span><span className={`text-xs font-bold flex items-center gap-1 ${color}`}><TrendingUp className={`w-3 h-3 ${isIncrease ? "" : "rotate-180"}`} />{pct > 0 ? "+" : ""}{pct}%</span></div>
                          <div className="space-y-1.5">
                            <div className="flex items-center gap-3"><span className="text-[10px] text-slate-500 w-14 text-right font-medium">Original</span><div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-slate-400 rounded-full transition-all duration-700" style={{ width: `${Math.min(origPct, 100)}%` }} /></div><span className="text-xs font-mono text-slate-600 w-20 text-right">{formatNumber(data.original)}</span></div>
                            <div className="flex items-center gap-3"><span className="text-[10px] text-slate-500 w-14 text-right font-medium">Adjusted</span><div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full ${barColor} rounded-full transition-all duration-700`} style={{ width: `${Math.min(adjPct, 100)}%` }} /></div><span className="text-xs font-mono font-bold text-slate-800 w-20 text-right">{formatNumber(data.adjusted)}</span></div>
                          </div>
                        </div>
                      );
                    })
                  ) : <div className="text-center py-6 text-slate-400 text-sm"><p>No comparison data. Adjust parameters to see comparisons.</p></div>}
                </div>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                  <h4 className="font-bold text-slate-900 text-sm mb-4 flex items-center gap-2"><LineChart className="w-4 h-4 text-[#4648d4]" /> Parameter Impact (%)</h4>
                  <div className="space-y-3">
                    {simulationResults.scenario_comparison && Object.entries(simulationResults.scenario_comparison).filter(([, data]) => data.change_percent !== undefined).map(([key, data]) => {
                      const pct = data.change_percent || 0;
                      const isPos = pct >= 0;
                      ...
                      return (
                        <div key={key} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 h-6 bg-slate-50 rounded-lg overflow-hidden relative">
                            <div className={`absolute top-0 h-full ${color} rounded-lg transition-all duration-700 flex items-center justify-end px-2`} style={{ width: `${barWidth}%`, left: isPos ? "0" : `${100 - barWidth}%` }}>
                ...
                <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                  <h4 className="font-bold text-slate-900 text-sm mb-4 flex items-center gap-2"><PieChart className="w-4 h-4 text-[#4648d4]" /> Value Comparison</h4>
                  <div className="space-y-3">
                    {simulationResults.scenario_comparison && Object.entries(simulationResults.scenario_comparison).filter(([, data]) => data.original !== undefined && data.adjusted !== undefined).slice(0, 6).map(([key, data]) => {
                      const max = Math.max(Math.abs(data.original), Math.abs(data.adjusted), 1);
                      return (
                        <div key={key}>
                          <div className="flex items-center justify-between mb-1"><span className="text-xs text-slate-600 font-medium">{formatKpiName(key)}</span><span className="text-[10px] text-slate-400">{formatNumber(data.adjusted)} vs {formatNumber(data.original)}</span></div>
                          <div className="flex items-center gap-1 h-4"><div className="h-full bg-slate-300 rounded-l-md transition-all duration-700" style={{ width: `${(Math.abs(data.original) / max) * 50}%` }} /><div className="h-full bg-[#4648d4] rounded-r-md transition-all duration-700" style={{ width: `${(Math.abs(data.adjusted) / max) * 50}%` }} /></div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="bg-gradient-to-br from-[#4648d4]/5 to-white rounded-2xl p-6 border border-[#4648d4]/20 shadow-sm">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <button onClick={() => onConfirmSimulation(simulationResults)} className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-semibold text-sm shadow-lg shadow-emerald-500/20 transition-all active:scale-[0.98] flex items-center gap-2 whitespace-nowrap"><CheckCircle className="w-4 h-4" /> Confirm & Proceed <ArrowRight className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          )}
...
export default function App() {
  ...
  const uniqueAgentNames = Array.from(new Set(messages.map((m) => m.agent).filter(Boolean)));
  const visibleAgents = uniqueAgentNames.map((name, idx) => {
    const styleObj = AGENT_STYLE_POOL[idx % AGENT_STYLE_POOL.length];
    return { ...styleObj, name, id: `dyn_agent_${idx}` };
  });
  ...
  const processFile = async (file) => {
    ...
    try {
      ...
      if (data.success && data.extracted_text) {
        ...
        setExtractedDocText(data.extracted_text || "[Gagal mengekstrak teks]");
  ...
  const handleConfirmSimulation = (simulationSummary) => {
    if (ws.current && ws.current.readyState === WebSocket.OPEN) {
      ws.current.send(JSON.stringify({ type: "confirm_simulation", simulation_summary: simulationSummary }));
    }
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-[#191c1d] font-sans antialiased selection:bg-[#e1e0ff] selection:text-[#07006c] relative">
        ...
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-red-100 animate-fadeIn">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center"><AlertOctagon className="w-5 h-5 text-red-500" /></div>
              <h3 className="text-lg font-bold text-slate-900">System Alert</h3>
            </div>
            <p className="text-sm text-slate-600 leading-relaxed mb-6">{ERROR_MESSAGES[apiError] || "Terjadi kesalahan pada sistem. Silakan coba lagi."}</p>
            <button onClick={() => setApiError(null)} className="w-full py-2.5 bg-slate-900 text-white rounded-xl font-semibold text-sm hover:bg-slate-800 transition-colors">Tutup</button>
          </div>
        </div>
      )}
        ...
        <div className="flex justify-between items-center w-full px-6 py-4 max-w-screen-2xl mx-auto">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex gap-6">
              <button onClick={() => setView("workspace")} className={`font-medium py-1 transition-colors ${view === "workspace" ? "text-[#4648d4] border-b-2 border-[#4648d4]" : "text-[#464554] hover:text-[#4648d4]"}`}>Workspace</button>
            </div>
          </div>
          <div className="flex items-center gap-4">
      ...
      <main className="relative">
          ...
          <div className="relative min-h-[calc(100vh-72px)] flex flex-col items-center px-6 py-20 md:py-28 hero-pattern">
            ...
            <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-10 border-t border-gray-200 pt-16 z-10">
              <div className="bg-white rounded-[32px] p-10 shadow-xl border border-gray-100 flex flex-col justify-between hover:shadow-2xl transition-all">
                <div>
                  <div className="flex items-center gap-5 mb-8"><div className="w-14 h-14 bg-[#4648d4]/10 text-[#4648d4] rounded-2xl flex items-center justify-center border border-[#4648d4]/20"><Brain className="w-7 h-7" /></div><div><h3 className="font-extrabold text-gray-950 text-2xl tracking-tight">Core Engine Specifications</h3><p className="text-sm text-gray-400 font-bold uppercase tracking-wider mt-0.5">Quantitative Accuracy Mode</p></div></div>
                  <ul className="space-y-6">
                    <li className="flex gap-4 items-start text-base text-gray-800 leading-relaxed font-medium"><span className="w-3 h-3 rounded-full bg-[#4648d4] mt-2.5 shrink-0" /><div><strong className="text-gray-950 font-extrabold text-lg block mb-0.5">Chain-of-Thought Enforcement</strong>Setiap agen WAJIB menuliskan rumus matematis sebelum menyebutkan angka final, mencegah halusinasi kalkulasi.</div></li>
                    <li className="flex gap-4 items-start text-base text-gray-800 leading-relaxed font-medium"><span className="w-3 h-3 rounded-full bg-[#4648d4] mt-2.5 shrink-0" /><div><strong className="text-gray-950 font-extrabold text-lg block mb-0.5">Fact Grounding Lock</strong>Angka dari prompt user (gaji, kapasitas, jam kerja) dianggap sumber kebenaran mutlak yang dilarang diubah oleh AI.</div></li>
                    <li className="flex gap-4 items-start text-base text-gray-800 leading-relaxed font-medium"><span className="w-3 h-3 rounded-full bg-[#4648d4] mt-2.5 shrink-0" /><div><strong className="text-gray-950 font-extrabold text-lg block mb-0.5">Document Intelligence</strong>Membaca dan menganalisis isi dokumen Word, Excel, PDF, dan gambar secara langsung untuk ekstraksi data kuantitatif.</div></li>
                  </ul>
                </div>
              </div>
              <div className="bg-white rounded-[32px] p-10 shadow-xl border border-gray-100 flex flex-col justify-between hover:shadow-2xl transition-all">
                <div>
                  <div className="flex items-center gap-5 mb-8"><div className="w-14 h-14 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center border border-emerald-100"><BarChart3 className="w-7 h-7" /></div><div><h3 className="font-extrabold text-gray-950 text-2xl tracking-tight">Argunex AI vs Regular Chatbots</h3><p className="text-sm text-gray-400 font-bold uppercase tracking-wider mt-0.5">Advanced Architecture Power</p></div></div>
                  <ul className="space-y-5">
                    <li className="flex gap-4 items-start text-base text-gray-800 leading-relaxed font-medium p-4 bg-gray-50 rounded-2xl border border-gray-100"><Cpu className="w-6 h-6 text-emerald-600 mt-0.5 shrink-0" /><div><strong className="text-gray-950 font-extrabold text-lg block mb-0.5">Math-Aware Multi-Agent Debate</strong>Bukan opini bebas, melainkan perdebatan terstruktur dengan verifikasi rumus di setiap langkah.</div></li>
                    <li className="flex gap-4 items-start text-base text-gray-800 leading-relaxed font-medium p-4 bg-gray-50 rounded-2xl border border-gray-100"><Shield className="w-6 h-6 text-emerald-600 mt-0.5 shrink-0" /><div><strong className="text-gray-950 font-extrabold text-lg block mb-0.5">Human-in-the-Loop Interruption</strong>Moderator hanya bertanya jika ada parameter kuantitatif yang benar-benar hilang, bukan mengulang hal yang sudah jelas.</div></li>
                    <li className="flex gap-4 items-start text-base text-gray-800 leading-relaxed font-medium p-4 bg-gray-50 rounded-2xl border border-gray-100"><FileText className="w-6 h-6 text-emerald-600 mt-0.5 shrink-0" /><div><strong className="text-gray-950 font-extrabold text-lg block mb-0.5">Document-Aware Analysis</strong>Bisa membaca isi file Excel, Word, PDF, dan gambar — bahkan via Paste (Ctrl+V) atau Drag & Drop.</div></li>
                  </ul>
                </div>
              </div>
            </div>
        ...
        {view === "workspace" && (
          <div className="flex-grow flex flex-col items-center justify-center px-6 py-16 relative">
            <div className="w-full max-w-3xl flex flex-col items-center text-center gap-4 mb-12">
              <h1 className="text-4xl font-bold tracking-tight text-[#191c1d]">How can <span className="text-[#4648d4]">Argunex AI</span> assist your workflow today?</h1>
              <p className="text-lg text-[#464554]">Upload, Paste, or Drag & Drop your documents here.</p>
            </div>
            <div className="w-full max-w-3xl relative">
              <div className={`bg-white rounded-3xl p-4 flex flex-col gap-4 border shadow-lg transition-all ${isDraggingOver ? "border-[#4648d4] border-2 shadow-[0_0_30px_rgba(70,72,212,0.2)]" : "border-[#e1e3e4]"}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
                {isDraggingOver && (
                ...
                <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); startProcess(); } }} onPaste={handlePaste} maxLength={MAX_INPUT_LENGTH} className="w-full border-none focus:ring-0 text-base bg-transparent resize-none p-2 placeholder:text-gray-400 outline-none h-32" placeholder="Describe your operational problem... (You can also Ctrl+V to paste an image here)" />
                <div className="flex justify-end px-2"><span className={`text-[11px] font-medium ${input.length >= MAX_INPUT_LENGTH ? "text-red-500" : "text-slate-400"}`}>{input.length}/{MAX_INPUT_LENGTH}</span></div>
                {selectedFile && (
                  <div className="mx-2 p-4 bg-slate-50 rounded-2xl border border-slate-200 animate-fadeIn">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div><p className="text-sm font-semibold text-slate-800">{selectedFile.name}</p><p className="text-[10px] text-slate-500">{getFileTypeInfo(selectedFile.name).label} • {(selectedFile.size / 1024).toFixed(1)} KB</p></div>
                      </div>
                      <button onClick={removeFile} className="text-red-500 hover:text-red-700 text-xs font-semibold p-1.5 rounded-lg hover:bg-red-50 transition-colors"><X className="w-4 h-4" /></button>
                    </div>
                    {isProcessingFile && <div className="flex items-center gap-2 mt-2 text-sm text-indigo-600"><Loader2 className="w-4 h-4 animate-spin" /><span className="font-medium">Membaca isi dokumen...</span></div>}
                    {fileUploadError && !isProcessingFile && <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700 flex items-start gap-2"><AlertTriangle className="w-3.5 h-3.5 mt-0.5 shrink-0" /><span>{fileUploadError}</span></div>}
                    {extractedDocText && !isProcessingFile && (
                      <div className="mt-2">
                        <div className="flex items-center gap-2 mb-1"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /><span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider">Dokumen berhasil dibaca • {extractedDocText.length.toLocaleString()} karakter</span></div>
                        <details className="mt-1"><summary className="text-xs text-slate-500 cursor-pointer hover:text-[#4648d4] font-medium transition-colors">👁️ Preview isi dokumen yang diekstrak</summary><div className="mt-2 p-3 bg-white rounded-xl border border-slate-200 max-h-48 overflow-y-auto"><pre className="text-xs text-slate-600 whitespace-pre-wrap font-mono leading-relaxed">{extractedDocText.substring(0, 2000)}{extractedDocText.length > 2000 ? "\n\n[... sisa teks dipotong untuk preview ...]" : ""}</pre></div></details>
                      </div>
                    )}
                  </div>
                )}
                <div className="flex justify-between items-center px-2">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    <span className="text-[11px] text-gray-400 font-medium leading-tight">Upload, Paste (Ctrl+V), or Drag & Drop • Word, Excel, PDF, PNG, JPG • Max 2MB</span>
                  </div>
                  <button onClick={startProcess} disabled={isProcessingFile || (!input.trim() && !extractedDocText)} className="bg-[#4648d4] text-white h-11 w-11 rounded-full flex items-center justify-center hover:scale-105 transition-all shadow-md shrink-0 disabled:bg-slate-300 disabled:cursor-not-allowed disabled:hover:scale-100"><span className="font-bold text-lg">↑</span></button>
                </div>
              </div>
            </div>
          </div>
        ...
        {view === "discussion" && (
          <div className="h-[calc(100vh-72px)] flex flex-col p-4 md:p-6 relative bg-slate-50">
            <div className="w-full bg-white rounded-2xl p-4 mb-4 shadow-sm border border-slate-200">
              <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><Activity className="w-4 h-4 text-[#4648d4]" /><span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Orchestration Progress</span></div><span className="text-xs font-bold text-[#4648d4]">{progress}%</span></div>
              <div className="w-full h-2.5 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-[#4648d4] to-[#6063ee] transition-all duration-700 rounded-full progress-glow" style={{ width: `${progress}%` }} /></div>
            </div>
            <div className="flex-1 flex gap-4 min-h-0">
              <div className="flex-1 bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden relative">
                <div className="absolute top-4 left-4 z-10 flex items-center gap-2 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-xl border border-slate-200 shadow-sm"><Network className="w-4 h-4 text-[#4648d4]" /><span className="text-xs font-bold text-slate-700">Multi-Agent Network</span></div>
                <div className="absolute top-4 right-4 z-10"><div className={`phase-indicator flex items-center gap-2 px-4 py-2 rounded-xl border text-xs font-bold shadow-sm ${getPhaseStyle(discussionPhase)}`}>{getPhaseIcon(discussionPhase)} {discussionPhase.toUpperCase()}</div></div>
                <NetworkGraph zoomLevel={zoomLevel} activeAgent={activeAgent} hoveredNode={hoveredNode} setHoveredNode={setHoveredNode} problemText={problemText} agents={visibleAgents} onNodeClick={handleAgentClick} />
              ...
              <div className="w-80 flex flex-col gap-4">
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex-1 overflow-y-auto">
                  <h3 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2"><Users className="w-4 h-4 text-[#4648d4]" /> Agent Status</h3>
                  <div className="space-y-3">
                    {visibleAgents.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full min-h-[120px] text-slate-400"><Activity className="w-5 h-5 animate-pulse mb-2 opacity-60" /><span className="text-xs font-medium italic">Awaiting AI network allocation...</span></div>
                    ) : (
                      visibleAgents.map((agent) => {
                        const agentMessages = messages.filter((m) => m.agent === agent.name);
                        const latestMsg = agentMessages[agentMessages.length - 1];
                        const status = activeAgent === agent.name ? "active" : "completed";
                        const displayText = () => {
                          if (latestMsg && latestMsg.text && latestMsg.text.trim().length > 3) return latestMsg.text.substring(0, 100) + (latestMsg.text.length > 100 ? "..." : "");
                          if (status === "active") return `Processing ${agent.name.toLowerCase()} analysis...`;
                          return "Analysis completed.";
                        };
                        return (
                          <div key={agent.id} onClick={() => handleAgentClick(agent.name)} className={`agent-card p-3 rounded-xl border cursor-pointer ${getAgentCardStyle(status)}`}>
                            <div className="flex items-center justify-between mb-1"><div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${getStatusDot(status)}`} /><span className="text-xs font-bold text-slate-700">{agent.name}</span></div>{getAgentStatusIcon(status)}</div>
                ...
                <div className="bg-slate-900 rounded-2xl p-4 shadow-lg border border-slate-800">
                  <div className="flex items-center gap-2 mb-2"><MessageSquare className="w-3.5 h-3.5 text-emerald-400" /><span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">System Trace</span></div>
                  <div className="h-24 overflow-y-auto"><p className="text-xs font-mono text-slate-300 leading-relaxed break-words">{currentLog}</p></div>
                  <div className="mt-2 flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /><span className="text-[10px] text-slate-500 font-mono">LIVE</span></div>
                </div>
              </div>
            </div>
            {isWaitingUser && (
              <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md z-40 flex items-center justify-center p-4 md:p-8">
                <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-slate-200 p-6 md:p-8 flex flex-col max-h-[85vh]">
                  <div className="flex items-center gap-4 mb-5"><div className="w-14 h-14 bg-amber-50 text-amber-600 rounded-2xl flex items-center justify-center shadow-lg shadow-amber-100"><AlertTriangle size={28} /></div><div><h2 className="text-xl font-extrabold tracking-tight text-slate-950">Moderator Interruption</h2><p className="text-xs text-slate-500 font-medium">Clarification required to proceed</p></div></div>
                  <div className="bg-amber-50/80 border border-amber-100 rounded-2xl p-4 mb-5 overflow-y-auto max-h-40"><p className="text-base text-slate-800 font-semibold leading-relaxed whitespace-normal break-words">{modQuestion && modQuestion.trim().length > 0 ? modQuestion : "Moderator memerlukan data kuantitatif tambahan untuk melanjutkan analisis. Mohon berikan parameter yang mungkin belum disebutkan sebelumnya."}</p></div>
                  <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendAnswer(); } }} maxLength={MAX_CLARIFICATION_LENGTH} className="w-full p-4 border border-slate-200 rounded-xl mb-2 text-base font-medium outline-none focus:ring-2 focus:ring-[#4648d4] bg-slate-50 resize-none h-28" placeholder="Provide the missing parameter..." />
                  <div className="flex justify-end mb-4"><span className={`text-[11px] font-medium ${input.length >= MAX_CLARIFICATION_LENGTH ? "text-red-500" : "text-slate-400"}`}>{input.length}/{MAX_CLARIFICATION_LENGTH}</span></div>
                  <button onClick={sendAnswer} className="w-full bg-[#4648d4] text-white py-3.5 rounded-xl font-bold hover:bg-[#6063ee] transition-all shadow-md text-base active:scale-[0.98]">Submit Response</button>
                </div>
              </div>
            )}
          </div>
        ...
        {/* SIMULATION */}
        {view === "simulation" && (
          <SimulationView simulationData={simulationData} simulationResults={simulationResults} onRunSimulation={handleRunSimulation} onConfirmSimulation={handleConfirmSimulation} onSkipSimulation={handleSkipSimulation} onBackToDiscussion={() => setView("discussion")} wsConnected={wsConnected} />
        )}
          ...
          <div className="max-w-7xl mx-auto py-12 px-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-8">
              <div className="bg-white rounded-[24px] p-8 shadow-sm border border-[#e1e3e4]">
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
                  <div className="flex items-center gap-3"><TrendingUp className="w-6 h-6 text-[#4648d4]" /><h1 className="text-3xl font-bold text-slate-900">Final Resolution Blueprint</h1></div>
                  <button onClick={() => setView("simulation")} className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-[#4648d4] transition-colors border border-slate-200 px-4 py-2 rounded-xl hover:bg-slate-50 shrink-0"><ArrowLeft className="w-4 h-4" /> Back to Simulation</button>
                </div>
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              <div className="bg-white rounded-[24px] p-6 border border-[#e1e3e4] shadow-sm">
                <div className="flex items-start gap-4 mb-6"><div className="w-12 h-12 rounded-xl bg-red-50 text-red-500 flex items-center justify-center"><FileText size={24} /></div><div><h3 className="font-bold text-base">Analysis_Report.pdf</h3><p className="text-xs text-gray-400">Ready to download</p></div></div>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setPdfPreviewUrl(`${API_BASE}${result.files.pdf}`)} disabled={!result.files.pdf} className="flex items-center justify-center gap-2 border border-gray-200 py-2.5 rounded-xl text-xs font-semibold hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"><Eye size={14} /> Preview</button>
                  <button onClick={() => forceDownload(`${API_BASE}${result.files.pdf}`, "Analysis_Report.pdf")} disabled={!result.files.pdf} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold text-center ${result.files.pdf ? "bg-[#4648d4] text-white hover:bg-[#3638b0]" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}><Download size={14} /> Download</button>
                </div>
              </div>
              <div className="bg-white rounded-[24px] p-6 border border-[#e1e3e4] shadow-sm">
                <div className="flex items-start gap-4 mb-6"><div className="w-12 h-12 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center"><Layers size={24} /></div><div><h3 className="font-bold text-base">Executive_Slides.pptx</h3><p className="text-xs text-gray-400">Multi-Slide Presentation Format</p></div></div>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => { setCurrentSlideIndex(0); setIsPptPreviewOpen(true); }} disabled={!result.files.ppt} className="flex items-center justify-center gap-2 border border-gray-200 py-2.5 rounded-xl text-xs font-semibold hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"><Eye size={14} /> Preview</button>
                  <button onClick={() => forceDownload(`${API_BASE}${result.files.ppt}`, "Executive_Slides.pptx")} disabled={!result.files.ppt} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold text-center ${result.files.ppt ? "bg-[#4648d4] text-white hover:bg-[#3638b0]" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}><Download size={14} /> Download</button>
                </div>
              </div>
            </div>
            {/* METRICS DASHBOARD */}
        ...
        {pdfPreviewUrl && (
          <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-6 backdrop-blur-sm">
            <div className="bg-white w-full h-full max-w-4xl rounded-2xl flex flex-col overflow-hidden">
              <div className="p-4 bg-gray-900 text-white flex justify-between items-center"><h3 className="font-bold text-sm">Live PDF Report Preview</h3><button onClick={() => setPdfPreviewUrl(null)} className="text-xs font-bold hover:bg-gray-800 px-3 py-1 rounded-lg">Close (X)</button></div>
        ...
        {isPptPreviewOpen && (
          <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-center p-4 backdrop-blur-md">
            <div className="w-full max-w-4xl flex flex-col gap-4">
              <div className="flex justify-between items-center text-white px-2"><p className="text-sm font-semibold tracking-wider uppercase opacity-80 flex items-center gap-2"><Layers size={16} className="text-[#4648d4]" /> Executive Slide Presentasi (Local Preview)</p><button onClick={() => setIsPptPreviewOpen(false)} className="bg-white/10 hover:bg-white/20 p-2 rounded-full"><X size={20} /></button></div>
              <div className="w-full aspect-[16/9] bg-white rounded-3xl shadow-2xl border border-gray-100 p-12 flex flex-col justify-between relative overflow-hidden">
                ...
                {pptSlides.length > 0 ? (
                  <>
                    <div><span className="text-[10px] font-bold text-[#4648d4] uppercase tracking-widest bg-[#4648d4]/10 px-3 py-1 rounded-full">Slide {currentSlideIndex + 1} of {pptSlides.length}</span><h2 className="text-3xl font-bold text-gray-900 mt-4 tracking-tight">{pptSlides[currentSlideIndex].title}</h2></div>
                    <div className="flex-1 flex flex-col justify-center my-6"><ul className="space-y-4">{pptSlides[currentSlideIndex].bullets.map((bullet, bIdx) => <li key={bIdx} className="text-lg text-gray-700 flex items-start gap-3 leading-relaxed"><span className="w-2 h-2 rounded-full bg-[#4648d4] mt-3 shrink-0" /><span>{bullet}</span></li>)}</ul></div>
                    <div className="flex justify-between items-center border-t border-gray-100 pt-4 text-xs text-gray-400 font-medium"><span>Argunex Intelligence Engine</span><span>Confidential Strategy Blueprint</span></div>
                  </>
              ...
              <div className="flex justify-between items-center px-4">
                <button disabled={currentSlideIndex === 0} onClick={() => setCurrentSlideIndex((p) => Math.max(0, p - 1))} className="flex items-center gap-2 px-5 py-2.5 bg-white rounded-xl text-sm font-bold text-gray-700 shadow-md hover:bg-gray-50 disabled:opacity-40 transition-all"><ChevronLeft size={16} /> Previous</button>
                <span className="text-sm text-white/80 font-semibold tracking-wider">Slide {currentSlideIndex + 1} / {pptSlides.length}</span>
                <button disabled={currentSlideIndex === pptSlides.length - 1} onClick={() => setCurrentSlideIndex((p) => Math.min(pptSlides.length - 1, p + 1))} className="flex items-center gap-2 px-5 py-2.5 bg-[#4648d4] text-white rounded-xl text-sm font-bold shadow-md hover:bg-[#6063ee] disabled:opacity-40 transition-all">Next <ChevronRight size={16} /></button>
              </div>
            </div>
          </div>
        )}
